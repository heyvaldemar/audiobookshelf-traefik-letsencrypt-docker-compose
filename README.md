...
Every release from v1.0.5 on carries three files made on GitHub's runner with a short-lived identity and no stored key: `audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.tar.gz`, a `git archive` of exactly the tree the tag points at; `audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json`, a keyless [Sigstore](https://www.sigstore.dev/) signature over it; and `audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl`, [SLSA](https://slsa.dev/) build provenance from the SLSA generator. To check them with nothing from this repository trusted:

```bash
cosign verify-blob audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --bundle audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.tar.gz.sigstore.json \
  --certificate-identity-regexp '^https://github.com/heyvaldemar/audiobookshelf-traefik-letsencrypt-docker-compose/' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com

slsa-verifier verify-artifact audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.tar.gz \
  --provenance-path audiobookshelf-traefik-letsencrypt-docker-compose-<tag>.intoto.jsonl \
  --source-uri github.com/heyvaldemar/audiobookshelf-traefik-letsencrypt-docker-compose
```
