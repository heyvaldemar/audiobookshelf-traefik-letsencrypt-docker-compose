#!/bin/bash
# End-to-end tests for the audiobookshelf-traefik-letsencrypt-docker-compose
# backup + restore flow.
#
# This application keeps everything it knows in one directory: absdatabase
# .sqlite, holding users, libraries and listening progress. There is no database
...
COMPOSE_PROJECT_NAME="${COMPOSE_PROJECT_NAME:-audiobookshelf}"
DOCKER_COMPOSE_FILE="${DOCKER_COMPOSE_FILE:-audiobookshelf-traefik-letsencrypt-docker-compose.yml}"
...
APP_CONTAINER="$(docker compose -f "$DOCKER_COMPOSE_FILE" -p "$COMPOSE_PROJECT_NAME" ps -aq audiobookshelf | head -n 1)"
[[ -n "$BACKUPS_CONTAINER" ]] || { echo "error: backups container not found" >&2; exit 1; }
[[ -n "$APP_CONTAINER" ]] || { echo "error: audiobookshelf container not found" >&2; exit 1; }
...
test_env_required() {
  grep -qE '^AUDIOBOOKSHELF_HOSTNAME=.' .env || { echo "  AUDIOBOOKSHELF_HOSTNAME is not set in .env" >&2; return 1; }
...
test_backup_carries_the_database() {
  # A tarball of /config that happens to contain a directory proves very little.
  # absdatabase.sqlite is the instance: users, libraries, and where every
  # listener stopped. An archive missing that one file restores into a server
  # that starts perfectly and has forgotten everyone.
  local f; f=$(list_backups | tail -1)
  bk "tar -tzf '$f' | grep -q '^config/absdatabase.sqlite'" \
    || { echo "  $f does not carry absdatabase.sqlite" >&2; return 1; }
...
test_restore_roundtrip() {
  ...
  COMPOSE_PROJECT_NAME="$COMPOSE_PROJECT_NAME" ./audiobookshelf-restore-config.sh "$(basename "$f")" > /dev/null \
    || { echo "  ./audiobookshelf-restore-config.sh failed" >&2; return 1; }
